module ModelHelper
    ...
    def guessFilePath(filePathIn)
        possibilities = [filePathIn, File.join(Rails.root, filePathIn)]
        filePath = possibilities.select {|p|
            File.exists?(p)
        }[0]

        if filePath.nil?
            raise "No file found at #{possibilities.join(', ')}"
        end
